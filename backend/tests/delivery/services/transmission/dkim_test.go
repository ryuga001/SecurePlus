...
func TestSignAddsSignature(t *testing.T) {
	signed, err := transmission.Sign([]byte(rawMessage), testConfig(t))
	if err != nil {
...
func TestSignPreservesExistingSignature(t *testing.T) {
	...
	signed, err := transmission.Sign([]byte(incoming), testConfig(t))
	if err != nil {
		t.Fatalf("Sign returned %v", err)
	}
...
func TestSignRejectsMissingKey(t *testing.T) {
	...
	if _, err := transmission.Sign([]byte(rawMessage), cfg); !errors.Is(err, deliveryutils.ErrPrivateKeyMissing) {
...
func TestSignRejectsInvalidKey(t *testing.T) {
	...
	if _, err := transmission.Sign([]byte(rawMessage), cfg); !errors.Is(err, deliveryutils.ErrPrivateKeyInvalid) {
...
func TestSignFallsBackToDefaultSelector(t *testing.T) {
	...
	signed, err := transmission.Sign([]byte(rawMessage), cfg)
	if err != nil {
		t.Fatalf("Sign returned %v", err)
	}
