...
import (
	...
	dto "dpdp-backend/internal/delivery/engine/incidentgenerator/dto"
)

type IncidentGenerator struct {
	writer dto.AuditWriter
}

func NewIncidentGenerator(writer dto.AuditWriter) *IncidentGenerator {
	return &IncidentGenerator{writer: writer}
}

func (g *IncidentGenerator) Record(ctx context.Context, result evaluation.EvaluationResult, message evaluation.MessageContext) error {
	return g.writer.Record(ctx, g.Generate(dto.GenerationInput{Message: message, Result: result}))
}
...
func (g *IncidentGenerator) Generate(input dto.GenerationInput) auditdto.Record {
