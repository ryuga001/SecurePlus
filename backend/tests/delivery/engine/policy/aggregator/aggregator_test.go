...
func policy(id int, name, mode string, values ...string) dto.PolicyRecord {
...
func TestBlocklistsAreUnioned(t *testing.T) {
	effective := aggregator.NewAggregator().Aggregate([]dto.PolicyRecord{
		policy(1, "A", utils.RestrictionBlock, "a.test"),
		policy(2, "B", utils.RestrictionBlock, "b.test"),
		policy(3, "C", utils.RestrictionBlock, "c.test", "a.test"),
	})
...
func TestAllowlistsAreIntersected(t *testing.T) {
	effective := aggregator.NewAggregator().Aggregate([]dto.PolicyRecord{
		policy(1, "A", utils.RestrictionAllow, "company.com", "partner.com"),
		policy(2, "B", utils.RestrictionAllow, "company.com", "vendor.com"),
	})
...
func TestEmptyAllowlistDoesNotEmptyTheIntersection(t *testing.T) {
	effective := aggregator.NewAggregator().Aggregate([]dto.PolicyRecord{
		policy(1, "A", utils.RestrictionAllow, "company.com", "partner.com"),
		policy(2, "B", utils.RestrictionNone),
		policy(3, "C", utils.RestrictionAllow, "company.com", "vendor.com"),
	})
...
func TestAggregationOfNoPoliciesImposesNothing(t *testing.T) {
	effective := aggregator.NewAggregator().Aggregate(nil)
...
func TestAttachmentRestrictionsAggregateIndependently(t *testing.T) {
	...
	effective := aggregator.NewAggregator().Aggregate([]dto.PolicyRecord{first, second})
