...
func NewDispatcher(ctx context.Context, engine Processor, relay Sender, recorder auditdto.Recorder) *Dispatcher {
	return &Dispatcher{engine: engine, relay: relay, recorder: recorder, ctx: ctx}
}

func (d *Dispatcher) Dispatch(msg EmailMessage, done func()) {
	d.wg.Add(1)
...
func (d *Dispatcher) complete(msg EmailMessage, result auditdto.Result) {
