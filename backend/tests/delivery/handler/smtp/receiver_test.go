...
func TestMessageID(t *testing.T) {
	...
	if got := smtp.MessageID(raw); got != "<abc@example.com>" {
...
func TestMessageIDMissing(t *testing.T) {
	...
	if got := smtp.MessageID(raw); got != "" {
		t.Fatalf("MessageID = %q, want empty", got)
	}
}

func TestMessageIDMalformed(t *testing.T) {
	if got := smtp.MessageID([]byte("not a message at all")); got != "" {
		t.Fatalf("MessageID = %q, want empty", got)
	}
}
...
func TestAuthorizeFromCache(t *testing.T) {
	...
	authorization, err := smtp.NewAuthorizer(cache, store).Authorize(context.Background(), "example.com")
	if err != nil {
...
func TestAuthorizeRejectsUnknownDomain(t *testing.T) {
	...
	_, err := smtp.NewAuthorizer(cache, store).Authorize(context.Background(), "stranger.test")
...
func TestAuthorizeSurfacesStoreFailure(t *testing.T) {
	...
	_, err := smtp.NewAuthorizer(cache, store).Authorize(context.Background(), "example.com")
