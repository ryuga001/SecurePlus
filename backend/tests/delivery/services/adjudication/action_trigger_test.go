...
func TestHighestPriorityActionWins(t *testing.T) {
	got := adjudication.ResolveAction([]dto.RuleMatch{
...
func TestNoMatchesResolvesToNone(t *testing.T) {
	if got := adjudication.ResolveAction(nil); got != utils.ActionNone {
...
func TestMatchOrderDoesNotChangeTheResult(t *testing.T) {
	...
	for index, matches := range orderings {
		if got := adjudication.ResolveAction(matches); got != utils.ActionBlock {
...
func TestFactoryResolvesEveryAction(t *testing.T) {
	factory := adjudication.DefaultActionFactory(nil)
...
func TestFactoryHasNoExecutorForNone(t *testing.T) {
	if executor, ok := adjudication.DefaultActionFactory(nil).For(utils.ActionNone); ok || executor != nil {
		t.Fatalf("For(NONE) = (%v, %v), want (nil, false)", executor, ok)
	}
}

func TestFactoryReportsUnknownAction(t *testing.T) {
	if executor, ok := adjudication.DefaultActionFactory(nil).For("SHRED"); ok || executor != nil {
