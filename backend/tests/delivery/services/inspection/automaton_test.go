...
func hits(t *testing.T, patterns []string, text string) map[int]int {
	...
	for _, hit := range inspection.BuildAutomaton(patterns).Find(text) {
		counts[hit.Index]++
	}
...
func TestAutomatonReportsOffsets(t *testing.T) {
	found := inspection.BuildAutomaton([]string{"card"}).Find("a card here")
...
func TestAutomatonIgnoresEmptyPatterns(t *testing.T) {
	found := inspection.BuildAutomaton([]string{""}).Find("anything")
