package incidentgenerator_test
...
import (
	...
	generatordto "dpdp-backend/internal/delivery/engine/incidentgenerator/dto"
...
func TestGeneratedRecordCarriesTheEnvelopeAndDecision(t *testing.T) {
	record := incidentgenerator.NewIncidentGenerator(&spyWriter{}).
...
func TestRecordDelegatesToTheAuditWriter(t *testing.T) {
	...
	if err := incidentgenerator.NewIncidentGenerator(writer).Record(context.Background(), result(), message()); err != nil {
...
func TestRecordSurfacesTheWriterFailure(t *testing.T) {
	...
	err := incidentgenerator.NewIncidentGenerator(writer).Record(context.Background(), result(), message())
...
func TestUpdateActionDelegatesTheOutcome(t *testing.T) {
	...
	err := incidentgenerator.NewIncidentGenerator(writer).UpdateAction(context.Background(), "corr-1", dto.ActionResult{
		Action: utils.ActionQuarantine,
