...
import (
	...
	rowdto "dpdp-backend/internal/delivery/dto/policyset"
...
func TestConcurrentMissesCollapseToOneQuery(t *testing.T) {
	...
	service := policy.NewPolicyCacheService(store, builder, nil, time.Minute)
...
func TestDifferentSendersDoNotCollapse(t *testing.T) {
	...
	service := policy.NewPolicyCacheService(store, &countingBuilder{}, nil, time.Minute)
...
func TestExpiredEntryIsRebuilt(t *testing.T) {
	store := &countingStore{rows: rows()}
	service := policy.NewPolicyCacheService(store, &countingBuilder{}, nil, time.Nanosecond)
...
func TestBuildFailurePropagates(t *testing.T) {
	...
	service := policy.NewPolicyCacheService(&countingStore{rows: rows()}, &countingBuilder{err: failure}, nil, time.Minute)
...
func TestSenderIsAssembledIntoAPolicySet(t *testing.T) {
	service := policy.NewPolicyCacheService(&countingStore{rows: rows()}, &countingBuilder{}, nil, time.Minute)
...
func TestSetKeyIsCaseInsensitiveOnSender(t *testing.T) {
	if policy.SetKey(1, "Alice@Example.com") != policy.SetKey(1, "alice@example.com") {
		t.Fatal("cache key must not vary with sender casing")
	}
	if policy.SetKey(1, "alice@example.com") == policy.SetKey(2, "alice@example.com") {
...
func assembled(t *testing.T, rows []rowdto.PolicyRuleRow) dto.PolicySet {
	...
	service := policy.NewPolicyCacheService(&countingStore{rows: rows}, builder, nil, time.Minute)
