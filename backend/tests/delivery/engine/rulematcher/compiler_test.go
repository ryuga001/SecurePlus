...
func TestCompilerRejectsOversizedPolicySet(t *testing.T) {
	...
	_, err := rulematcher.NewCompiler(stubAggregator{}, 4).Build(dto.PolicySet{CustomerID: 1, Rules: rules})
...
func TestCompilerCountsPolicies(t *testing.T) {
	set, err := rulematcher.NewCompiler(stubAggregator{}, utils.MaxRules).Build(dto.PolicySet{
		CustomerID: 1,
