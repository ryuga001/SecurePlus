...
func bodyOf(t *testing.T, raw string) string {
	...
	parsed, err := inspection.Parse([]byte(raw))
	if err != nil {
...
func TestParsesPlainTextMessage(t *testing.T) {
	parsed, err := inspection.Parse([]byte(
...
func TestVisibleTextDropsAttributeValues(t *testing.T) {
	got := inspection.VisibleText(`<div class="confidential" title="secret">plain</div>`)
...
func TestMalformedMessageDegradesInsteadOfFailingHard(t *testing.T) {
	parsed, err := inspection.Parse([]byte("this is not a message at all"))
...
func TestExtensionOf(t *testing.T) {
	...
	for filename, want := range cases {
		if got := inspection.ExtensionOf(filename); got != want {
