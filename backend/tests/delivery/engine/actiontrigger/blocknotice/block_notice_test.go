...
func service(t *testing.T, store *stubTemplates, relay *spyRelay) *blocknotice.BlockNoticeService {
	t.Helper()

	return blocknotice.NewBlockNoticeService(store, stubConfigs{cfg: delivery.TenantConfig{Domain: "example.com"}}, relay)
}
...
func TestNoticeSurfacesASigningFailure(t *testing.T) {
	...
	svc := blocknotice.NewBlockNoticeService(templates(), stubConfigs{err: failure}, &spyRelay{})
