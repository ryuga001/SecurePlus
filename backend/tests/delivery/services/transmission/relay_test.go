...
func TestClassify(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			code, permanent := transmission.Classify(tc.err)
...
func TestNextBackoff(t *testing.T) {
	first := time.Minute
	second := transmission.NextBackoff(first, 5, 15*time.Minute)

	if second != 5*time.Minute {
		t.Fatalf("second delay = %v, want 5m", second)
	}

	third := transmission.NextBackoff(second, 5, 15*time.Minute)
	if third != 15*time.Minute {
		t.Fatalf("third delay = %v, want the 15m cap", third)
	}

	if capped := transmission.NextBackoff(third, 5, 15*time.Minute); capped != 15*time.Minute {
		t.Fatalf("delay grew past the cap: %v", capped)
	}

	if zero := transmission.NextBackoff(time.Minute, 0, time.Hour); zero != time.Minute {
		t.Fatalf("multiplier below one changed the delay: %v", zero)
	}
}

func TestResolverHonoursOverride(t *testing.T) {
	resolver := transmission.NewResolver(time.Second, false, "mailpit:1025")
...
func TestResolverOverrideWithoutPortUses25(t *testing.T) {
	resolver := transmission.NewResolver(time.Second, false, "relay.internal")
...
func TestResolverUnknownDomain(t *testing.T) {
	resolver := transmission.NewResolver(2*time.Second, false, "")
